 ...
 * Date:            Feb 21, 2017  
 * Comments:     
 * This file contains drivers for the 240RGB x 320 TFT LCD display. Because this
 * can use either serial or parallel connections, both serial and parallel 
 * functions are included in this one header file. Theses functions are 
 * essentially the same except for the prefix 'Para_' in front of the 
 * parallel functions. 
 * 
 * Revision history: 
 * May 14, 2018:
 * Added initialization and driver functions for 8-bit parallel connection to the 
 * display.
 * 
 */
...
void Para_FillScreen_ILI9341(int colour);
...
void Para_FillRec_ILI9341(long int x, long int y, long int w, long int h, long int colour) {
    ...
    Para_SetAddrWindow_ILI9341(x, y, x + w - 1, y + h - 1);
    ...
    for (y = h; y > 0; y--) {
        for (x = w; x > 0; x--) {
            data_Mask = (colour_hi << 1); // Shift data to bit position 1-8.
...
void Para_FillScreen_ILI9341(int colour) {
    Para_FillRec_ILI9341(0, 0, ILI9341_TFTWIDTH, ILI9341_TFTHEIGHT, colour);
}
